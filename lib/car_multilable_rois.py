...
def read_object_labels(root, set):
    path_labels = os.path.join(root, 'split')
    labeled_data = dict()
...
class ImageFolder(data.Dataset):
    def __init__(self, root, set, transform=None, target_transform=None):
        ...
        self.path_rois = os.path.join(root,'car_rois500')
        self.set = set
        ...
        path_csv = os.path.join(self.root, 'path_label_csv')
        ...
        if not os.path.exists(file_csv):
            ...
            write_rois_pt(self.path_rois, labeled_data.keys())

        self.classes = object_categories
        self.images = read_object_labels_csv(file_csv)

        print('[dataset] car classification set=%s number of classes=%d  number of images=%d' % (
            set, len(self.classes), len(self.images)))

    def __getitem__(self, index):
        path, target = self.images[index]
        img = Image.open(os.path.join(self.path_images, path + '.jpg')).convert('RGB')
        rois = torch.load(os.path.join(self.path_rois, path + '.pt'))

        if self.transform is not None:
            img,rois = self.transform(img,rois)
        if self.target_transform is not None:
            target = self.target_transform(target)

        return (img, rois), target
...
def write_rois_pt(path_rois, imagename):

    print('[dataset] transfer rois.txt to rois.pt file')
    for path in imagename:
        ##rois = torch.FloatTensor([[0.0000, 50.0000,  50.0000,  150.0000,  150.0000 ],[0.0000, 100.0000,  100.0000,  200.0000,  200.0000]])
        rois = torch.from_numpy(np.loadtxt(os.path.join(path_rois, path + '.txt'),dtype=float)).type(torch.FloatTensor)
        torch.save(rois, os.path.join(path_rois, path + '.pt'))
